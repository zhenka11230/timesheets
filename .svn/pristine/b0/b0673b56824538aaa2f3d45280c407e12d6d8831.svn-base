...
$fields['dblLoading'] = $db->escapeSimple(trim(@$_REQUEST['dblLoading']));
$fields['bytHourStart'] = $db->escapeSimple(trim(@$_REQUEST['bytHourStart']));
$fields['bytMinuteStart'] = $db->escapeSimple(trim(@$_REQUEST['bytMinuteStart']));
$fields['bytHourEnd'] = $db->escapeSimple(trim(@$_REQUEST['bytHourEnd']));
$fields['bytMinuteEnd'] = $db->escapeSimple(trim(@$_REQUEST['bytMinuteEnd']));
$fields['bytRestrictClockInLimit'] = $db->escapeSimple(trim(@$_REQUEST['bytRestrictClockInLimit']));
...
$pager->conditions[] = array(
	'AND' => array(
		...
		'dblLoading' => array($fields['dblLoading'],'eq'),
		'bytHourStart' => array($fields['bytHourStart'],'eq'),
		'bytMinuteStart' => array($fields['bytMinuteStart'],'eq'),
		'bytHourEnd' => array($fields['bytHourEnd'],'eq'),
		'bytMinuteEnd' => array($fields['bytMinuteEnd'],'eq'),
		'bytRestrictClockInLimit' => array($fields['bytRestrictClockInLimit'],'eq'),
		)
	);
$pager->table = Shift::$tableName;
$pager->RunSearch();
...
$(document).ready(function() 
{
	...
	{
		if ($('input.tblShifts:checked').size() == 0)
		{
		...
		{
			var link='controller.php?action=delete_tblShift';
			$('input.tblShifts:checked').each(function(){
				link += '&id[]='+$(this).val();
			});
	...
	$('#tblShifts_all').click(function(){
			$('.tblShifts').attr('checked', $('#tblShifts_all').attr('checked'));
		});
	$('.tblShifts').click(function(){
			if ($(this).attr('checked') == false && $('#tblShifts_all').attr('checked'))
				$('#tblShifts_all').removeAttr('checked');
		});
});
</script>

<div class="page_title">Manage TblShifts</div>

<form method="get" action="<?=$_SERVER['PHP_SELF'];?>">
	<input type="hidden" name="view" value="tblShifts"/>
	...
	<div class="tabbed_section_body">
		...
		<div style="search_field_wrapper" id="sf_dblLoading">
			<div class="search_field_label">DblLoading:</div>
			<div class="search_field">double</div>
		</div>
		<div style="clear: both;"></div>
		<div style="search_field_wrapper" id="sf_bytHourStart">
			<div class="search_field_label">BytHourStart:</div>
			<div class="search_field">short</div>
		</div>
		
		<div style="search_field_wrapper" id="sf_bytMinuteStart">
			<div class="search_field_label">BytMinuteStart:</div>
			<div class="search_field">short</div>
		</div>
		<div style="clear: both;"></div>
		<div style="search_field_wrapper" id="sf_bytHourEnd">
			<div class="search_field_label">BytHourEnd:</div>
			<div class="search_field">short</div>
		</div>
		
		<div style="search_field_wrapper" id="sf_bytMinuteEnd">
			<div class="search_field_label">BytMinuteEnd:</div>
			<div class="search_field">short</div>
		</div>
		<div style="clear: both;"></div>
		<div style="search_field_wrapper" id="sf_bytRestrictClockInLimit">
			<div class="search_field_label">BytRestrictClockInLimit:</div>
			<div class="search_field">short</div>
		</div>
		...
		<div style="search_field_wrapper">
			...
			<div class="search_field_label">
				...
				<input type="button" value="Reset Form" class="button btn-reload" style="width:100px;" onClick="location='?view=tblShifts&reset'"/>
			</div>
		</div>
		<div style="clear: both;"></div>
	</div>
	...
	<table width="100%">
		<tr>
			<?php if (accessible_permission("add_tblShift","view")) { ?>
				<td style="padding-right:10px;border-right:solid 1px maroon;width:124px;">
					<input type="button" value="Add TblShift" onClick="location='?view=add_tblShift'" class="button btn-add" style="width:124px;"/>
				</td>
			<?php } ?>
			<td align="center">
				<?php $pager->getPager();?>
	...
	<table class="grid-def">
		<tr class="grid-row_header">
			<?php if (accessible_permission("delete_tblShift_model","model")) { ?>
				<td width="50" align="center">Delete</td>
				<!--<td width="40" align="center" style="padding:0"><input type="checkbox" id="tblShifts_all" title="Select all items on this page"/></td>-->
			<?php } ?>
			<td>
			  StrName
		...
		foreach($pager->results as $row)
		{
			$i++;
			$shift = new Shift();
			$shift->setPropertyValues($row);
			?>
			<tr class="grid-row<?= ($i%2) ?>">
				<?php if (accessible_permission("delete_tblShift_model","model")) { ?>
					<td align="center">
						<a href='controller.php?action=delete_tblShift&id=<?=$shift->lngID;?>&popup_destination=<?=@$popup_destination;?>' onClick='return confirm("Are you sure that you want to delete this tblShift?");'><img src='<?=COMMON_FILES_PATH;?>images/icons/delete.gif' /></a>
						<!--<input type="checkbox" class="tblShifts" name="id[]" value="<?=$shift->lngID;?>"/>-->
					</td>
				<?php } ?>
				<td>
					<?php if (accessible_permission("add_tblShift","view")) {
						echo "<a href='?view=add_tblShift&id=".$shift->lngID."'>".$shift->strName."</a>";
					}
					else
						echo $shift->strName;
				?>
				</td>
				<!--<td><?php //echo $shift->strName;?></td>-->
			</tr>
		<?php
		}
		if ($pager->total_results == 0) 
