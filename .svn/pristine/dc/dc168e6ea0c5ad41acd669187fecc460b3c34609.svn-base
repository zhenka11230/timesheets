...
// pager fields
$fields['sort_by'] = (isset($_REQUEST['sort_by']) ? $db->escapeSimple(trim($_REQUEST['sort_by'])) : "lngBulletinID_a");
...
$pager->table = BulletinAssignment::$tableName;
$pager->RunSearch();
...
$(document).ready(function() 
{
	...
	{
		if ($('input.tblBulletinAssignments:checked').size() == 0)
		{
		...
		{
			var link='controller.php?action=delete_tblBulletinAssignment';
			$('input.tblBulletinAssignments:checked').each(function(){
				link += '&id[]='+$(this).val();
			});
	...
	$('#tblBulletinAssignments_all').click(function(){
			$('.tblBulletinAssignments').attr('checked', $('#tblBulletinAssignments_all').attr('checked'));
		});
	$('.tblBulletinAssignments').click(function(){
			if ($(this).attr('checked') == false && $('#tblBulletinAssignments_all').attr('checked'))
				$('#tblBulletinAssignments_all').removeAttr('checked');
		});
});
</script>

<div class="page_title">Manage TblBulletinAssignments</div>

<form method="get" action="<?=$_SERVER['PHP_SELF'];?>">
	<input type="hidden" name="view" value="tblBulletinAssignments"/>
	...
	<div class="tabbed_section_body">
		...
		<div style="search_field_wrapper">
			...
			<div class="search_field_label">
				...
				<input type="button" value="Reset Form" class="button btn-reload" style="width:100px;" onClick="location='?view=tblBulletinAssignments&reset'"/>
			</div>
		</div>
		<div style="clear: both;"></div>
	</div>
	...
	<table width="100%">
		<tr>
			<?php if (accessible_permission("add_tblBulletinAssignment","view")) { ?>
				<td style="padding-right:10px;border-right:solid 1px maroon;width:215px;">
					<input type="button" value="Add TblBulletinAssignment" onClick="location='?view=add_tblBulletinAssignment'" class="button btn-add" style="width:215px;"/>
				</td>
			<?php } ?>
			<td align="center">
				<?php $pager->getPager();?>
	...
	<table class="grid-def">
		<tr class="grid-row_header">
			<?php if (accessible_permission("delete_tblBulletinAssignment_model","model")) { ?>
				<td width="50" align="center">Delete</td>
				<!--<td width="40" align="center" style="padding:0"><input type="checkbox" id="tblBulletinAssignments_all" title="Select all items on this page"/></td>-->
			<?php } ?>
			<td>
			  LngBulletinID
				<input type="image" onclick="sort_by.value='lngBulletinID_a';" src='<?=COMMON_FILES_PATH;?>images/icons/arrow_up_16.gif' width="10" height="10" />
				<input type="image" onclick="sort_by.value='lngBulletinID_d';" src='<?=COMMON_FILES_PATH;?>images/icons/arrow_down_16.gif' width="10" height="10" />
			</td>
			<!--td>bulletinAssignment->lngBulletinID</td-->
		</tr>
		...
		{
			$i++;
			$bulletinAssignment = new BulletinAssignment();
			$bulletinAssignment->setPropertyValues($row);
			?>
			<tr class="grid-row<?= ($i%2) ?>">
				<?php if (accessible_permission("delete_tblBulletinAssignment_model","model")) { ?>
					<td align="center">
						<a href='controller.php?action=delete_tblBulletinAssignment&id=<?=$bulletinAssignment->lngID;?>&popup_destination=<?=@$popup_destination;?>' onClick='return confirm("Are you sure that you want to delete this tblBulletinAssignment?");'><img src='<?=COMMON_FILES_PATH;?>images/icons/delete.gif' /></a>
						<!--<input type="checkbox" class="tblBulletinAssignments" name="id[]" value="<?=$bulletinAssignment->lngID;?>"/>-->
					</td>
				<?php } ?>
				<td>
					<?php if (accessible_permission("add_tblBulletinAssignment","view")) {
						echo "<a href='?view=add_tblBulletinAssignment&id=".$bulletinAssignment->lngID."'>".$bulletinAssignment->lngBulletinID."</a>";
					}
					else
						echo $bulletinAssignment->lngBulletinID;
				?>
				</td>
				<!--<td><?php //echo $bulletinAssignment->lngBulletinID;?></td>-->
			</tr>
		<?php
		}
		if ($pager->total_results == 0) 
