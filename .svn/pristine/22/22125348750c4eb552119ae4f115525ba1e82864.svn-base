...
{
	public static $tableName = "tblHolidays";
	public $exists = false;
	...
	/** [[Column=lngMonth, DataType=int, Description=LngMonth]]*/
	public $lngMonth;

	/** [[Column=lngDay, DataType=int, Description=LngDay]]*/
	public $lngDay;

	/** [[Column=dblCreditedHours, DataType=float, Description=DblCreditedHours]]*/
	public $dblCreditedHours;

	/** [[Column=blnRecurring, DataType=bit, Description=BlnRecurring, Required=true]]*/
	public $blnRecurring;

	/** [[Column=lngYear, DataType=int, Description=LngYear]]*/
	public $lngYear;
	...
	public function getHolidays($args=array())
	{
		global $db;
		...
		$query = "SELECT * 
			...
			WHERE 1=1 
				...
				".(@$args['lngMonth'] != '' ? " AND lngMonth={$args['lngMonth']} " : "")."
				".(@$args['lngDay'] != '' ? " AND lngDay={$args['lngDay']} " : "")."
				".(@$args['dblCreditedHours'] != '' ? " AND dblCreditedHours={$args['dblCreditedHours']} " : "")."
				".(@$args['blnRecurring'] != '' ? " AND blnRecurring={$args['blnRecurring']} " : "")."
				".(@$args['lngYear'] != '' ? " AND lngYear={$args['lngYear']} " : "")."
			ORDER BY strName";
