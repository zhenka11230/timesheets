...
$pager->table = Department::$tableName;
$pager->RunSearch();
...
$(document).ready(function() 
{
	...
	{
		if ($('input.tblDepartments:checked').size() == 0)
		{
		...
		{
			var link='controller.php?action=delete_tblDepartment';
			$('input.tblDepartments:checked').each(function(){
				link += '&id[]='+$(this).val();
			});
	...
	$('#tblDepartments_all').click(function(){
			$('.tblDepartments').attr('checked', $('#tblDepartments_all').attr('checked'));
		});
	$('.tblDepartments').click(function(){
			if ($(this).attr('checked') == false && $('#tblDepartments_all').attr('checked'))
				$('#tblDepartments_all').removeAttr('checked');
		});
});
</script>

<div class="page_title">Manage Departments</div>

<form method="get" action="<?=$_SERVER['PHP_SELF'];?>">
	<input type="hidden" name="view" value="tblDepartments"/>
	...
	<div class="tabbed_section_body">
		...
		<div style="search_field_wrapper">
			...
			<div class="search_field_label">
				...
				<input type="button" value="Reset Form" class="button btn-reload" style="width:100px;" onClick="location='?view=tblDepartments&reset'"/>
			</div>
		</div>
		<div style="clear: both;"></div>
	</div>
	...
	<table width="100%">
		<tr>
			<?php if (accessible_permission("add_tblDepartment","view")) { ?>
				<td style="padding-right:10px;border-right:solid 1px maroon;width:159px;">
					<input type="button" value="Add Department" disabled="disabled" onClick="location='?view=add_tblDepartment'" class="button btn-add" style="width:159px;"/>
				</td>
			<?php } ?>
			<td align="center">
				<?php $pager->getPager();?>
	...
	<table class="grid-def">
		<tr class="grid-row_header">
			<?php if (accessible_permission("delete_tblDepartment_model","model")) { ?>
				<td width="50" align="center">Delete</td>
				<!--<td width="40" align="center" style="padding:0"><input type="checkbox" id="tblDepartments_all" title="Select all items on this page"/></td>-->
			<?php } ?>
			<td>
			  Name
		...
		foreach($pager->results as $row)
		{
			...
			$department->setPropertyValues($row);
			?>
			<tr class="grid-row<?= ($i%2) ?>">
				<?php if (accessible_permission("delete_tblDepartment_model","model")) { ?>
					<td align="center">
						<a href='controller.php?action=delete_tblDepartment&id=<?=$department->lngID;?>&popup_destination=<?=@$popup_destination;?>' onClick='return confirm("Are you sure that you want to delete this tblDepartment?");'><img src='<?=COMMON_FILES_PATH;?>images/icons/delete.gif' /></a>
						<!--<input type="checkbox" class="tblDepartments" name="id[]" value="<?=$department->lngID;?>"/>-->
					</td>
				<?php } ?>
				<td>
					<?php if (accessible_permission("add_tblDepartment","view")) {
						echo "<a href='?view=add_tblDepartment&id=".$department->lngID."'>".$department->strName."</a>";
					}
					else
						echo $department->strName;
				?>
				</td>
				<!--<td><?php //echo $department->strName;?></td>-->
			</tr>
		<?php
		}
		if ($pager->total_results == 0) 
